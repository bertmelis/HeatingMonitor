...
void handleUpdateEnd() {
  webUpdateServer.sendHeader("Connection", "close");
  if (Update.hasError()) {
    webUpdateServer.send(502, "text/plain", Update.errorString());
  } else {
    webUpdateServer.sendHeader("Refresh", "10");
    webUpdateServer.sendHeader("Location", "/");
    webUpdateServer.send(307);
    ESP.restart();
  }
}
...
void handleUpdate() {
  ...
  if (webUpdateServer.hasArg("size")) {
    fsize = webUpdateServer.arg("size").toInt();
  }
  HTTPUpload &upload = webUpdateServer.upload();
...
void initWebUpdate() {
  ...
  webUpdateServer.on("/favicon.ico", HTTP_GET, []() {
    webUpdateServer.sendHeader("Content-Encoding", "gzip");
    webUpdateServer.send_P(200, "image/x-icon", favicon_ico_gz, favicon_ico_gz_len);
  });
  webUpdateServer.onNotFound([]() {
    webUpdateServer.send(200, "text/html", indexHtml);
  });
